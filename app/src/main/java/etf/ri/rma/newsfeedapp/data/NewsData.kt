...
object NewsData {
    ...
    fun initializeHardcodedIfNeeded() {
        ...
        val hardcoded = listOf(
            NewsItem(
                ...
                tags = emptyList() // imageTags is now handled by the 'tags' property, which is List<TagEntity>
            ),
            NewsItem(
                news = NewsEntity( // Wrap in NewsEntity
                ...
                tags = emptyList() // Empty list of TagEntity
            ),
            NewsItem(
                news = NewsEntity( // Wrap in NewsEntity
    ...
    fun getAllNews(): List<NewsItem> {
        ...
        return (initialNews + fetchedNews).distinctBy { it.news.uuid + it.news.category }
    }


    fun getByCategory(category: String): List<NewsItem> {

        return getAllNews().filter { it.news.category.equals(category, ignoreCase = true) }
    }

    fun addItem(news: NewsItem, toCategory: String) {

        val exists = fetchedNews.any { it.news.uuid == news.news.uuid && it.news.category == toCategory }
        if (!exists) {

            fetchedNews.add(news.copy(news = news.news.copy(category = toCategory, isFeatured = false)))
        }
    }
    ...
    fun addAllIfNew(newItems: List<NewsItem>) {
        newItems.forEach { newItem ->
            ...
            val exists = fetchedNews.any { it.news.uuid == newItem.news.uuid && it.news.category == newItem.news.category }
            if (!exists) {

                fetchedNews.add(newItem.copy(news = newItem.news.copy(isFeatured = false)))
            }
        }
    }
    ...
    fun updateTagsForImageUrl(url: String, tags: List<String>) {
        (initialNews + fetchedNews).filter { it.news.imageUrl == url }.forEach { item ->
            val newTagEntities = tags.map { TagEntity(value = it) }

            item.tags.toMutableList().apply {
                clear()
                addAll(newTagEntities)
            }
